...
/**
 * Servlet implementation class fstudentteachingreport
 */
@WebServlet("/dir1/fstudentteachingreport")
public class fstudentteachingreport extends HttpServlet {
...
protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try
		{
			...
			HashMap<Integer, FeedbackReportBean> m=dao.getStudentTeachingReport(fid);

			if(m.get(1).getTotal()==0){
				request.getRequestDispatcher("femptyreport.jsp").forward(request, response);
			}else{

				ArrayList<FStudentTeachingBean> list=dao.getStudentTeachingDetailReport(fid);
...
//				request.setAttribute("q12a5", q12a5);
		    	...
		    	request.getRequestDispatcher("fstudentteachingreport.jsp").forward(request, response);
			}
		...
		{
			e.printStackTrace();
			response.sendRedirect("fstudentteachingreport.jsp?success=3");
		}
