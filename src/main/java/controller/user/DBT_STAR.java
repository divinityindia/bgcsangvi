...
/**
 * Servlet implementation class DBT_STAR
 */
@WebServlet("/dir1/DBT_STAR")
public class DBT_STAR extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(bean.isDBT_STAR()){
			try{
				...
				request.getRequestDispatcher("DBT_STAR.jsp").forward(request, response);
			}
			catch(Exception e)
	        {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
       ...
       try{
            ...
            int id=dao.addDoc(docName, "DBT_STAR");
            ...
            if(id != 0)
            {
            	...
            	dao2.uploadFileToBucket(Credentials, bucketName, "IQAC/DBT_STAR/"+id+"/"+docName, image, metadata);


            	out.println("SUCCESS");
                session.setAttribute("result", "1");
                response.sendRedirect("DBT_STAR");
            }
            else
            {
