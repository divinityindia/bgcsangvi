...
/**
 * Servlet implementation class AcademicCommitteeController
 */
@WebServlet("/dir1/AcademicCommitteeController")
public class AcademicCommitteeController extends HttpServlet {
		...
		if(bean.isAcademiccalender())
		{
			...
			try
			{
				...
				String filename="";
				 ...
				 AcademicCommitteeBean bean2=new AcademicCommitteeBean();
		       ...
		       // image=images.get(key);
		    	...
		    	{
		    	 AcademicCommitteeDao dao=new AcademicCommitteeDao();
				 int id=dao.addCommittee(fields.get("title"),filename);
			    	metadata.setContentLength(Long.valueOf(image.available()));
			    	metadata.setContentType("application/pdf");
			    	s3client.uploadfile(Credentials, bucketName, "AcademicCommittee/"+id+"_"+filename, image, metadata);
		    	}
		    	...
		    	response.sendRedirect("AcademicCommitteeController");
