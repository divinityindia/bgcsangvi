...
/**
 * Servlet implementation class deptPlacement
 */
@WebServlet("/dir1/deptPlacement")
public class deptPlacement extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException 
	{
		...
		if(bean.isUGC_BSR())
		{
			try
			{
				...
				RequestDispatcher rd=request.getRequestDispatcher("deptplacement.jsp");
			...
			StaffDepResultDao dao2=new StaffDepResultDao();
                if(did!=0)
                {
				request.setAttribute("det",dao2.getAllDepPlacement1(did));
                }
                else
                {
                	request.setAttribute("det",dao2.getAllDepPlacement());
				}
		...
		if(bean.isUGC_BSR())
		{
			try
			{
		       ...
		       // image=images.get(key);
		    	...
		    	s3client.uploadfile(Credentials, bucketName, "addPlacement/"+fields.get("did")+"_"+filename, image, metadata);
		    	}
		      ...
		      dao.addDeptPlacement(result);
		    	
		        
		    	System.out.println("In side deptResult post method");
		    	
		    	response.sendRedirect("deptPlacement");
