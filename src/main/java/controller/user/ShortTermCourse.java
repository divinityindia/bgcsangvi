...
/**
 * Servlet implementation class ShortTermCourse
 */
@WebServlet("/dir1/ShortTermCourse")
public class ShortTermCourse extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(bean.isCourse()){
			try{
				...
				request.getRequestDispatcher("ShortTermCourse.jsp").forward(request, response);
			}
			catch(Exception e)
	        {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
       ...
       try{
            ...
            while (iter.hasNext())
            {
               ...
               switch(fieldName)
               {
                  ...
                  case "file": isp=item.getInputStream();
                       ...
                       break;
                  case "status": bean.setStatus(item.getString());
                       break;
                  case "fees": bean.setFees(item.getString());
                       break;
            ...
            int resultID=dao.AddShortTermCourse(bean);
            ...
            if(resultID != 0)
            {
            	...
            	if(null != isp)
            	{
                	...
                	dao2.uploadFileToBucket(Credentials, bucketName, "short_term_course/"+SID+"/"+bean.getDocumentname(), image, metadata);
