...
/**
 * Servlet implementation class DepCourseStructUpdate
 */
@WebServlet("/dir1/DepCourseStructUpdate")
public class DepCourseStructUpdate extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException
	{
		...
		if(bean.isUGC_BSR())
		{
			try
			{
				...
				RequestDispatcher rd=request.getRequestDispatcher("deptcourseStruct.jsp");
				request.setAttribute("dep",dao.getDeptList());
				 int did=Integer.parseInt((String) session.getAttribute("DID"));
		    DepCorseStructDao dao2=new DepCorseStructDao();

		    if(did !=0)
            {
				request.setAttribute("det",dao2.getAllDepcourseStr1(did));
            }
            else
            {
         	   request.setAttribute("det",dao2.getAllDepcourseStr());
			   }
		...
		if(bean.isUGC_BSR())
		{
			try
			{
		       ...
		       // image=images.get(key);
		    	...
		    	s3client.uploadfile(Credentials, bucketName, "AddDepCourseStr/"+fields.get("didname")+"_"+filename, image, metadata);
		    	}
		    	DeptCourseStructure structure=new DeptCourseStructure();
		    	structure.setDeptid(Integer.parseInt(fields.get("didname")));
		    	structure.setCorsname(fields.get("title"));
                structure.setCorsedetl(fields.get("dtl"));
                structure.setYear(fields.get("yr"));
                structure.setDocument(filename);
		    	
                int dsid=Integer.parseInt(fields.get("srid"));
		    	
		         DepCorseStructDao dao=new DepCorseStructDao();
    		      dao.updateDeptCourse(structure, dsid);
		    	
		        
		    	System.out.println("In side deptResult post method");
		    	
		    	response.sendRedirect("DepCourseStructUpdate");
		    	
		    	
		    	 /*out.println("<script type=\"text/javascript\">");
				   out.println("alert('Staff Achiv Added Sucessfully:)');");
				   out.println("location='DepCourseStructUpdate';");
				   out.println("</script>");*/
