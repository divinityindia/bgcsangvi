...
/**
 * Servlet implementation class Add_CommitteeEventReportController
 */
@WebServlet("/dir1/Add_CommitteeEventReportController")
public class Add_CommitteeEventReportController extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException 
	{
		...
		if(bean.isGallery()){
			try
			{
				...
				request.setAttribute("cmdtls", dao.getAllCommittee());
			 RequestDispatcher rd=request.getRequestDispatcher("Add_CommitteeEventReport.jsp");
			 request.setAttribute("cerdtl",dao.getAllCmtEvntReports());
			...
			try
			{
				...
				String filename="";
				  ...
				  while (iterator.hasNext()) 
				  {
				   ...
				   int cmid=Integer.parseInt(fields.get("cmid"));
				   int evid=Integer.parseInt(fields.get("evid"));
				   
				   
				   ActivityDao dao=new ActivityDao();
			     int id=dao.addCmtEvntReport(cmid,evid, filename);
		       ...
		       // image=images.get(key);
		    	...
		    	s3client.uploadfile(Credentials, bucketName, "AddCommitteeEventReport/"+id+"_"+filename, image, metadata);
		    	}
		    	
		    	session.setAttribute("erMsg", "Committee Event Report Added Successfully....");		    	
