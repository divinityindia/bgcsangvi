...
/**
 * Servlet implementation class fparentcollegereport
 */
@WebServlet("/dir1/fparentcollegereport")
public class fparentcollegereport extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try
		{
			...
			HashMap<Integer, FeedbackReportBean> m=dao.getParentCollegeReport(fid);

			if(m.get(1).getTotal()==0){
				request.getRequestDispatcher("femptyreport.jsp").forward(request, response);
			}else{

				ArrayList<FParentCollegeBean> list=dao.getParentCollegeDetailReport(fid);
...
//				request.setAttribute("q15a5", q15a5);
		    	...
		    	request.getRequestDispatcher("fparentcollegereport.jsp").forward(request, response);
			}
		...
		{
			e.printStackTrace();
			response.sendRedirect("fparentcollegereport.jsp?success=3");
		}
