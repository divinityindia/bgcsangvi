...
/**
 * Servlet implementation class DeptRserchPublicationUpdate
 */
@WebServlet("/dir1/DeptRserchPublicationUpdate")
public class DeptRserchPublicationUpdate extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException 
	{
		...
		if(bean.isUGC_BSR())
		{
			try
			{
				...
				RequestDispatcher rd=request.getRequestDispatcher("deptstaffresearchPubl.jsp");
				request.setAttribute("dep",dao.getDeptList());
				
			    StaffResPublicationDao dao2=new StaffResPublicationDao();

				request.setAttribute("det",dao2.getAllStaffResPub());
		...
		if(bean.isUGC_BSR())
		{
			try
			{
		       ...
		       // image=images.get(key);
		    	...
		    	s3client.uploadfile(Credentials, bucketName, "AddDepResPub/"+fields.get("didname")+"_"+filename, image, metadata);
		    	}
		    	StaffResarch resarch=new StaffResarch();
		    	resarch.setDeptid(Integer.parseInt(fields.get("didname")));
		    	resarch.setTitle(fields.get("title"));
		    	resarch.setDetalis(fields.get("dtl"));
		    	resarch.setDocument(filename);
		    	int srid=Integer.parseInt(fields.get("srid"));
               StaffResPublicationDao dao=new StaffResPublicationDao();
		    	dao.updateDeptResPubUpdate(resarch, srid);
		    	
		        
		    	System.out.println("In side deptreserch post method");
		    	
		    	response.sendRedirect("DeptRserchPublicationUpdate");
