...
/**
 * Servlet implementation class DST_FIST
 */
@WebServlet("/dir1/DST_FIST")
public class DST_FIST extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(bean.isDST_FIST()){
			try{
				...
				request.getRequestDispatcher("DST_FIST.jsp").forward(request, response);
			}
			catch(Exception e)
	        {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
       ...
       try{
            ...
            int id=dao.addDoc(docName, "DST_FIST");
            ...
            if(id != 0)
            {
            	...
            	dao2.uploadFileToBucket(Credentials, bucketName, "IQAC/DST_FIST/"+id+"/"+docName, image, metadata);


            	out.println("SUCCESS");
                session.setAttribute("result", "1");
                response.sendRedirect("DST_FIST");
            }
            else
            {
