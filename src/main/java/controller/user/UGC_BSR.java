...
/**
 * Servlet implementation class UGC_BSR
 */
@WebServlet("/dir1/UGC_BSR")
public class UGC_BSR extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(bean.isUGC_BSR()){
			try{
				...
				request.getRequestDispatcher("UGC_BSR.jsp").forward(request, response);
			}
			catch(Exception e)
	        {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
       ...
       try{
            ...
            int id=dao.addDoc(docName, "UGC_BSR");
            ...
            if(id != 0)
            {
            	...
            	dao2.uploadFileToBucket(Credentials, bucketName, "IQAC/UGC_BSR/"+id+"/"+docName, image, metadata);


            	out.println("SUCCESS");
                session.setAttribute("result", "1");
                response.sendRedirect("UGC_BSR");
            }
            else
            {
