...
/**
 * Servlet implementation class InfraPic
 */
@WebServlet("/dir1/InfraPic")
public class InfraPic extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException 
	{
		...
		if(bean.isUGC_Community_College()){
			try
			{
			 RequestDispatcher rd=request.getRequestDispatcher("InfraPic.jsp");
			 InfraPicDao dao=new InfraPicDao();
			 request.setAttribute("dtl",dao.getAllInfraPic());
		...
		if(bean.isUGC_Community_College())
		{
			...
			try
			{
		       ...
		       // image=images.get(key);
		    	...
		    	if(image != null)
		    	{
		    		 InfraPicBean bean2=new InfraPicBean();
					  bean2.setImgtype(fields.get("type"));
					  bean2.setTitle(fields.get("title"));
					  bean2.setImage(filename);
					  
					  InfraPicDao dao=new InfraPicDao();
					  int id=dao.addInfraPic(bean2);
					  
		    	metadata.setContentLength(Long.valueOf(image.available()));
		    	s3client.uploadfile(Credentials, bucketName, "Infrapic/"+id+"_"+filename, image, metadata);
		    	}
		    	
		    	
		    	session.setAttribute("erMsg", "Infra Image Added Successfully");
		     response.sendRedirect("InfraPic");
		    ...
		    /* DeptDao dao2=new DeptDao();
		     ...
		     response.sendRedirect("DeptAboutDtlController?id="+getid+"&did="+fields.get("did")+"&dnm="+dpname);*/
		    	...
		    	System.out.println("In side InfraPic post method");
