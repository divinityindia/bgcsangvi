...
/**
 * Servlet implementation class AcedemicCalenderController
 */
@WebServlet("/dir1/AcedemicCalenderController")
public class AcedemicCalenderController extends HttpServlet {
		...
		if(bean.isAcademiccalender())
		{
			...
			try
			{
				...
				String filename="";
				 ...
				 AcedemicCalenderBean bean2=new AcedemicCalenderBean();
		       ...
		       // image=images.get(key);
		    	...
		    	{
		    	 AcedemicCalenderesDao dao=new AcedemicCalenderesDao(); 
				 int id=dao.addAcedemicCalender(fields.get("title"),filename);
			    	metadata.setContentLength(Long.valueOf(image.available()));
			    	metadata.setContentType("application/pdf");
			    	s3client.uploadfile(Credentials, bucketName, "AcedemicCalenderes/"+id+"_"+filename, image, metadata);
		    	}
		    	...
		    	response.sendRedirect("AcedemicCalenderController");
