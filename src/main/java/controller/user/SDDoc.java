...
/**
 * Servlet implementation class SDDoc
 */
@WebServlet("/dir1/SDDoc")
public class SDDoc extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException 
	{
		...
		if(bean.isWelfare()){
			try
			{
				...
				request.getRequestDispatcher("SDdoc.jsp").forward(request, response);
			}
			catch(Exception e)
	        {
			...
			if(bean.isWelfare())
			{
				try
				{
			       ...
			       // image=images.get(key);
			    	...
			    	SddocDao dao=new SddocDao();
			    	int count=dao.addSDDoc(bean2);
			    	
			    	if(image != null)
			    	{
			    	metadata.setContentLength(Long.valueOf(image.available()));
			    	metadata.setContentType("application/pdf");
			    	s3client.uploadfile(Credentials, bucketName, "AddSDDoc/"+count+"_"+filename, image, metadata);
			    	}
