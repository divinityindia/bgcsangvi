...
/**
 * Servlet implementation class DeptResultUpdate
 */
@WebServlet("/dir1/DeptResultUpdate")
public class DeptResultUpdate extends HttpServlet {
		...
		if(bean.isUGC_BSR())
		{
			try
			{
		       ...
		       // image=images.get(key);
		    	...
		    	s3client.uploadfile(Credentials, bucketName, "AddDepResult/"+fields.get("didname")+"_"+filename, image, metadata);
		    	}
		    	StaffResult result=new StaffResult();
		    	result.setDeptid(Integer.parseInt(fields.get("didname")));
		    	result.setExamname(fields.get("title"));
		    	result.setYear(fields.get("yr"));
		    	result.setDocument(filename);
		    	
		    	
		    	int daid=Integer.parseInt(fields.get("srid"));
		    	
		      StaffDepResultDao dao=new StaffDepResultDao();
		      dao.updateDeptResult(result, daid);
		    	
		        
		    	System.out.println("In side deptResult post method");
		    	response.sendRedirect("DeptResultUpdate");
		    	
		    	
		    	
		    	 /*out.println("<script type=\"text/javascript\">");
				   out.println("alert('Staff Achiv Added Sucessfully:)');");
				   out.println("location='DeptResultUpdate';");
				   out.println("</script>");*/
