...
/**
 * Servlet implementation class UpdateDepDtl
 */
@WebServlet("/dir1/UpdateDepDtl")
public class UpdateDepDtl extends HttpServlet {
			...
			try
			{
				con = DBConnection.createConnection();
				  ...
				  int id=Integer.parseInt(request.getParameter("id"));
					...
					request.setAttribute("dtl", dao2.getDepDtl(id));
			...
			try
			{
				con = DBConnection.createConnection();
				  ...
				  String detl=request.getParameter("area");
				   ...
				   pst=con.prepareStatement("update depthomeDel set deptid=?,deptname=?,heading=?,details=? where did=?");
			    	
				   pst.setInt(1,dpid);
				   pst.setString(2,depnm);
				   pst.setString(3,heading);
				   pst.setString(4,detl);
				   pst.setInt(5,id);
			    	pst.executeUpdate();
