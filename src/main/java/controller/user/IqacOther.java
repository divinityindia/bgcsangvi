...
/**
 * Servlet implementation class OtherIqac
 */
@WebServlet("/dir1/IqacOther")
public class IqacOther extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(bean.isUGC_CPE()){
			try{
				...
				request.getRequestDispatcher("IqacOther.jsp").forward(request, response);
			}
			catch(Exception e)
	        {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
       ...
       try{
            ...
            int id=dao.addDoc(docName, "IqacOther");
            ...
            if(id != 0)
            {
            	...
            	dao2.uploadFileToBucket(Credentials, bucketName, "IQAC/IqacOther/"+id+"/"+docName, image, metadata);


            	out.println("SUCCESS");
                session.setAttribute("result", "1");
                response.sendRedirect("IqacOther");
            }
            else
            {
