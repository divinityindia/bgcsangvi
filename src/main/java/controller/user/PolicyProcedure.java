...
/**
 * Servlet implementation class PolicyProcedure
 */
@WebServlet("/dir1/PolicyProcedure")
public class PolicyProcedure extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException 
	{
		...
		if(bean.isUGC_Community_College()){
			try
			{
			 RequestDispatcher rd=request.getRequestDispatcher("PolicyProedure.jsp");
			 PolicyProcedureDao dao=new PolicyProcedureDao();
			 request.setAttribute("dtl",dao.getAllPolicyProcedure());
		...
		if(bean.isUGC_Community_College())
		{
			...
			try
			{
				...
				String filename="";
				  ...
				  PolicyProcedureBean bean2=new PolicyProcedureBean();
				    bean2.setDpmenu(fields.get("title"));
		       ...
		       // image=images.get(key);
		    	...
		    	{
		    	 PolicyProcedureDao dao=new PolicyProcedureDao();
				 int id=dao.addPolicyProcedure(bean2,fields.get("title"));
			    	metadata.setContentLength(Long.valueOf(image.available()));
			    	metadata.setContentType("application/pdf");
			    	s3client.uploadfile(Credentials, bucketName, "PolicyProcedure/"+id+"_"+filename, image, metadata);
		    	}
		    	...
		    	response.sendRedirect("PolicyProcedure");
