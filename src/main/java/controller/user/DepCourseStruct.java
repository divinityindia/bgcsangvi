...
/**
 * Servlet implementation class DepCourseStruct
 */
@WebServlet("/dir1/DepCourseStruct")
public class DepCourseStruct extends HttpServlet {
		...
		if(bean.isUGC_BSR())
		{
			try
			{
		       ...
		       // image=images.get(key);
		    	...
		    	s3client.uploadfile(Credentials, bucketName, "AddDepCourseStr/"+fields.get("did")+"_"+filename, image, metadata);
		    	}
		    	DeptCourseStructure structure=new DeptCourseStructure();
		    	structure.setDeptid(Integer.parseInt(fields.get("did")));
		    	structure.setCorsname(fields.get("cname"));
                structure.setCorsedetl(fields.get("cdetl"));
                structure.setYear(fields.get("year"));
                structure.setDocument(filename);
		    	
		    	
		         DepCorseStructDao dao=new DepCorseStructDao();
    		      dao.addDeptcourseStr(structure);
		    	
		        
		    	System.out.println("In side deptResult post method");
		    	
		    	response.sendRedirect("DepCourseStruct");
