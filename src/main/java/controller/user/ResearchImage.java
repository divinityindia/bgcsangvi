...
/**
 * Servlet implementation class ResearchImage
 */
@WebServlet("/dir1/ResearchImage")
public class ResearchImage extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException 
	{
		...
		if(bean.isUGC_BSR())
		{
			try
			{
				...
				ResearchImgUploadDao dao2=new ResearchImgUploadDao();
				
				if(did!=0)
				{
				request.setAttribute("dtl",dao2.getAllResImg1(did));
				}
				else
				{
					request.setAttribute("dtl",dao2.getAllResImg());
				}
		...
		if(bean.isUGC_BSR())
		{
			try
			{
				...
				String filename="";
				  ...
				  ResearchImgUploadBean bean2=new ResearchImgUploadBean();
				  
				  int id1=Integer.parseInt(fields.get("did"));
				  DeptDao dao=new DeptDao();
				  String dpnm=dao.DeptName(id1);
				  System.out.println("Deptname=="+dpnm+" "+id1);
				  bean2.setDeptid(id1);
				  bean2.setDeptname(dpnm);
				  bean2.setImgtype(fields.get("type"));
				  bean2.setImgtitle(fields.get("imgnm"));
				  bean2.setImage(filename);
				  
				  ResearchImgUploadDao dao2=new ResearchImgUploadDao();
				  int id=dao2.addResImg(bean2);
		       ...
		       // image=images.get(key);
		    	...
		    	s3client.uploadfile(Credentials, bucketName, "ResearchImg/"+id+"_"+filename, image, metadata);
		    	}
		     ...
		     response.sendRedirect("ResearchImage");
		    ...
		    /* DeptDao dao2=new DeptDao();
		     ...
		     response.sendRedirect("DeptAboutDtlController?id="+getid+"&did="+fields.get("did")+"&dnm="+dpname);*/
		    	...
		    	System.out.println("In side ResearchImage post method");
