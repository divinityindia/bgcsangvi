...
/**
 * Servlet implementation class DBTStarCollegeNotices
 */
@WebServlet("/dir1/DBTStarCollegeNotices")
public class DBTStarCollegeNotices extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException 
	{
		...
		if(bean.isDBT_STAR())
		{
			try
			{
				...
				request.getRequestDispatcher("DBTStarCollegeNotices.jsp").forward(request, response);
			}
			catch(Exception e)
	        {
		       ...
		       // image=images.get(key);
		    	...
		    	DbtStarNoticesDao dao=new DbtStarNoticesDao();
				 int id=dao.addDBTStarNotices(bean2);
			    	...
			    	s3client.uploadfile(Credentials, bucketName,"DBT_Star_Notices/"+id+"_"+filename, image, metadata);
		    	
		    	response.sendRedirect("DBTStarCollegeNotices");
			}
			catch(Exception e)
	        {
				System.out.println("error= "+e);
