...
/**
 * Servlet implementation class ProspectusController
 */
@WebServlet("/dir1/ProspectusController")
public class ProspectusController extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException 
	{
		...
		if(bean.isAcademiccalender()){
			try
			{
			 RequestDispatcher rd=request.getRequestDispatcher("AddProspectus.jsp");
			...
			try
			{
				...
				String filename="";
				  ...
				  while (iterator.hasNext()) 
				  {
				    ...
				    ProspectusBean bean2=new ProspectusBean();
		       ...
		       // image=images.get(key);
		    	...
		    	if(image != null)
		    	{
		    		int id=1;
		    	 ProspectusDao dao=new ProspectusDao();
				  dao.updateProspectus(fields.get("title"),filename,id);
			    	metadata.setContentLength(Long.valueOf(image.available()));
			    	metadata.setContentType("application/pdf");
			    	s3client.uploadfile(Credentials, bucketName, "Prospectus/"+id+"_"+filename, image, metadata);
		    	}
		    	...
		    	response.sendRedirect("ProspectusController");
