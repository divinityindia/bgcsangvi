 ...
 */
@WebServlet("/dir1/UpdateTimeTable")
		...
		if(bean.isExam()){
			try
			{
				...
				TimeTableDao dao=new TimeTableDao();
                  dao.deleteTimeTable(ttid);
                  session.setAttribute("erMsg", "Time Table Deleted Successfully");
  		    	response.sendRedirect("AddTimeTable");
			}
			catch(Exception e)
	        {
			...
			try
			{
				...
				String filename="";
				  ...
				  TimeTableBean bean2=new TimeTableBean();
				   ...
				   bean2.setClsname(fields.get("clss"));
				   bean2.setYear(fields.get("yr"));
				   bean2.setSem(fields.get("sem"));
				   bean2.setTitle(fields.get("tlt"));
				   bean2.setDocument(filename);
				   bean2.setTtid(Integer.parseInt(fields.get("ttid")));
		       ...
		       // image=images.get(key);
		    	...
		    	if(image != null)
		    	{
		    		dao.updateTimeTableWithDoc(bean2);
		    	metadata.setContentLength(Long.valueOf(image.available()));
		    	metadata.setContentType("application/pdf");
		    	s3client.uploadfile(Credentials, bucketName, "AddTimeTable/"+bean2.getTtid()+"_"+filename, image, metadata);
		    	System.out.println("Indise with Doc.....");
		    	}
		    	else
		    	{
					dao.updateTimeTableWithOutDoc(bean2);
					System.out.println("Indise withOut Doc.....");
				}
		    	session.setAttribute("erMsg", "Time Table Updated Successfully");
		    	response.sendRedirect("AddTimeTable");
