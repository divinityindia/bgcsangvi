...
<head>
	...
	<script>
		function puValsInModel(aqid,year,document)
		{ 
	...
	<style>
		...
		.planeTbl tr:HOVER {
  		...
  		<div  style="border:1px solid #aaa;padding:10px;border-radius:5px;text-align: center">
	  		...
	  		<form action="Aqar" method="post"  enctype="multipart/form-data" onsubmit="return chkPDF()">
	  			<table  class="planeTbl" style="width:100%">
	  				...
	  				<tr>
	  					...
	  					<td colspan="4"><input type="submit" value=" ADD " class="btn" /></td>
	  				</tr>
	  			</table>
...
<!-- 					<input type="submit" value=" ADD " class="btn" /> -->
  		...
  		<br/>
  		<b>Already Uploaded AQAR Files:</b>
  			...
  			<thead>
  				...
  				<c:forEach var="cat" varStatus="row" items="${det}">
  				 <tr>
  				     <td>${row.count}</td>
  				     <td>${cat.year}</td>
  				     <td><a href="${s3path}AQAR/${cat.document}" target="_blank">${cat.document}</a></td>
  				     <td><a href="DeleteAQAR?id=${cat.aqid}&doc=${cat.document}">Delete</a></td>
  				     <td><input type="button" onclick="puValsInModel('${cat.aqid}','${cat.year}','${cat.document}')" value="  Edit  " class="green-btn" data-toggle="modal" data-target="#myModal"/></td>
  				 </tr>
  				</c:forEach>
  			</thead>
  			<tbody>
		      ...
		      <div class="modal-content">
		        ...
		        <div class="modal-body" style="text-align: center">
	  			...
	  			function chkPDF2(){
					...
					<form action="AQARUpdate" method="post"  enctype="multipart/form-data" onsubmit="return chkPDF2()">
		        	<table  class="planeTbl" style="width:100%">
		        	    
		        		<tr>
		  					<td width="18%">Year <span class="req">*</span></td>
		        			<td width="32%">
		        			<input type="text" class="form-control" style="resize:none" name="year" id="year" readonly="readonly"> 	
		        			</td>
		  					<td>Upload Document</td>
		  					<td>
		  						<input type="file"  name="file" id="file-modal" required="required"/>
		  					</td>
		  				</tr>
		  				<tr>
		  					<td></td>
		  					<td>
		  						<input type="hidden" name="aqid" id="aqid"/>
