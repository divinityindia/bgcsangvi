...
<head>
	...
	$( document ).ready(function() {
		...
		if($('#resultUpdate').val() == '1'){
			$("#newsUpdateSucc").click();
		}
		if($('#resultDelete').val() == '1'){
			$("#newsDeleteSucc").click();
		}
			...
			success:function(data){
				...
				for (var key in data.hm) {
					($('#myTable tbody')).append('<tr >'
						...
						+'<td>'+data.hm[key][1]+'</td>'
						+'<td>'+data.hm[key][2]+'</td>'
						+'<td><a href="'+s3path+'news/'+key+'/'+data.hm[key][4]+'" target="_blank">'+data.hm[key][4]+'</a></td>'
						+'<td>'+data.hm[key][5]+'</td>'
						+'<td>'+data.hm[key][6]+'</td>'
						+'<td>'+data.hm[key][8]+'</td>'
						+'<td>'+data.hm[key][9]+'</td>'
						+'<td> <input type="checkbox" class="delChkBox" name="chkBox" value="'+key+'" onchange="chkDisableDelBtn()"/> </td>'
						+'<td> <input type="button" onclick="puValsInModel(\''+key+'\',\''+data.hm[key][0]+'\',\''+data.hm[key][2]+'\',\''+data.hm[key][5]+'\',\''+data.hm[key][3]+'\',\''+data.hm[key][6]+'\',\''+data.hm[key][7]+'\',\''+data.hm[key][8]+'\',\''+data.hm[key][9]+'\')" value="  Edit  " class="green-btn" data-toggle="modal" data-target="#myModal"/> </td>'
						+'</tr>');
	...
	<script>
		function puValsInModel(NID, DID, title, date, details, status, link,active,edate){
			$("#nid-modal").val(NID);
			$("#dept-modal").val(DID);
			...
			$("#link-modal").val(link);
			$("#blink-model").val(active);
			$("#edate-modal").val(edate);
		}
  ...
  <div class="container" style="padding:10px">
	...
	<style>
		...
		.planeTbl tr:HOVER {
  		...
  		<h3 style="line-height: 0px;">News Manager</h3>
  		<br/>
  			...
  			<table class="planeTbl" style="width:100%">
  				<tr>
  					...
  					<td width="10%">News Title <span class="req">*</span></td>
  					<td width="40%"><input type="text" class="form-control" name="title" required="required"/></td>
  				</tr>
  				<tr>
  					<td>Start Date <span class="req">*</span></td>
  					<td><input type="text" class="form-control"  id="date" name="sdate" required="required"/></td>
  				...
  				<tr>
  					<td>News Blink<span style="font-size: 0.8em">(To view Blinking online)</span>  <span class="req">*</span></td>
  					<td>
  					...
  					<td>End Date</td>
  					<td><input type="text" class="form-control" name="edate" id="edate" required="required"/></td>
  				</tr>
  				<tr>
  					<td><input type="hidden" value="${s3path}" id="s3path"></td>
  		...
  		<table id="myTable" class="Vscroll">
  			<thead>
  				<tr>
  					<th style="color: #fff; background: #002e3b;border:1px solid #003135; ">News ID</th>
  					<th style="color: #fff; background: #002e3b;border:1px solid #003135; ">Department </th>
  					<th style="color: #fff; background: #002e3b;border:1px solid #003135; ">News Title</th>
  					...
  					<th style="color: #fff; background: #002e3b;border:1px solid #003135; ">EndDate</th>
		        ...
		        <div class="modal-body" style="text-align: center">
		        	...
		        	<table  class="planeTbl" style="width:100%">
		        		<tr>
		        			...
		        			<td width="32%">
		        				<select class="form-control" required="required" name="dept" id="dept-modal">
		  							<option value="" selected="selected">~~ Choose~~</option>
		  							<c:forEach var="dept" items="${depts}" >
											<option value="${dept.key}">${dept.value}</option>
		  							</c:forEach>
		        			...
		        			<td width="13%">News Title <span class="req">*</span></td>
		  					<td width="37%"><input type="text" class="form-control" name="title" required="required" id="title-modal"/></td>
		        		</tr>
		        		<tr>
		  					<td>Start Date <span class="req">*</span></td>
		  					<td><input type="text" class="form-control"  id="date-modal" name="sdate" required="required"/></td>
		  					<td rowspan="2">Datails</td>
		  				...
		  				<tr>
		  					<td>Status <span style="font-size: 0.75em">(To view online)</span> <span class="req">*</span></td>
		  					<td>
		  					...
		  					<td>Link if any</td>
		  					<td><input type="text" class="form-control" name="link" id="link-modal"/></td>
		  				</tr>
		  				<tr>
  					<td>News Blink<span style="font-size: 0.8em">(To view Blinking online)</span>  <span class="req">*</span></td>
  					<td>
  						<select class="form-control" name="active" required="required" id="blink-model">
  							<option value="">Choose</option>
  							<option value="Yes">Yes</option>
  							<option value="No">No</option>
  						</select>
  					...
  					<td>End Date</td>
  					<td><input type="text" class="form-control" name="edate" id="edate-modal" required="required"/></td>
  				</tr>
		  				<tr>
		  					<td></td>
		  					<td>
		  						<input type="hidden" name="NID" id="nid-modal"/>
...
<!-- 		        </div> -->
		...
		<button type="button" id="newsCrSucc" style="display:none" class="btn btn-info btn-lg" data-toggle="modal" data-target="#myModal-success">Open Large Modal</button>
		      ...
		      <div class="modal-content">
		        <div class="modal-body" style="text-align: center">
					<br/><br/>
		          	...
		          	<h3 style="color:green">News created successfully</h3>
		  ...
		  <button type="button" id="newsUpdateSucc" style="display:none" class="btn btn-info btn-lg" data-toggle="modal" data-target="#myModal-success-update">Open Large Modal</button>
		      ...
		      <div class="modal-content">
		        <div class="modal-body" style="text-align: center">
					<br/><br/>
		          	...
		          	<h3 style="color:green">News updated successfully</h3>
		  ...
		  <button type="button" id="newsDeleteSucc" style="display:none" class="btn btn-info btn-lg" data-toggle="modal" data-target="#myModal-success-delete">Open Large Modal</button>
		      ...
		      <div class="modal-content">
		        <div class="modal-body" style="text-align: center">
					<br/><br/>
		          	...
		          	<h3 style="color:green">News deleted successfully</h3>
