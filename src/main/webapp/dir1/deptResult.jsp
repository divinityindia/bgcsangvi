	...
	<style>
		...
		.planeTbl tr:HOVER {
  		...
  		<div  style="border:1px solid #aaa;padding:10px;border-radius:5px;text-align: center">
	  		...
	  		<form action="deptResult" method="post"  enctype="multipart/form-data" onsubmit="return chkPDF()">
  			...
  			<thead>
  				...
  				<c:forEach var="cat" varStatus="row" items="${det}">
  				 <tr>
  				     ...
  				     <td>${cat.examname}</td>
  				     <td>${cat.year}</td>
  				     <td><a href="${s3path}AddDepResult/${cat.deptid}_${cat.document}" target="_blank">${cat.document}</a></td>
