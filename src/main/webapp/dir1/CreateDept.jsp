...
<head>
			...
			success:function(data){
				...
				for (var key in data.al) {
					($('#myTable tbody')).append('<tr >'
						+'<td>'+ (+key + +1) +'</td>'
						+'<td style="text-align:left;padding-left:15px;">'+data.al[key][1]+'</td>'
						+'<td> <input type="button" onclick="puValsInModel(\''+data.al[key][0]+'\',\''+data.al[key][1]+'\')" value="  Edit  " class="green-btn" data-toggle="modal" data-target="#myModal"/> </td>'
						+'</tr>');
	...
	<script>
		function puValsInModel(Did, dept){
			$('#modelResult').html('');
			$("#did").val(Did);
			$("#dept").val(dept);
		}
		...
		function updateDept(){
			$.ajax({
				type:"GET",
				url:"../UpdateDept",
				data:{id:$("#did").val(),dept:$("#dept").val()},
				success:function(data){
					var result=data.result;
					if(result == 1){
						$('#modelResult').html('<b style="color:darkgreen">Record Edited Successfully.</b>')
					}
					else{
						$('#modelResult').html('<b style="color:red">Something went wrong. Try again</b>')
					}
	  		...
	  		<br/>
	  			...
	  			<table  class="planeTbl" style="width:100%">
	  				<tr>
	  					...
	  					<td><input type="text" name="dept" required class="form-control" /></td>
	  				</tr>
	  				<tr>
	  					<td></td>
  		...
  		<table id="myTable" class="Vscroll">
  			<thead>
  				<tr>
  					...
  					<th style="color: #fff; background: #002e3b;border:1px solid #003135; ">Department Name</th>
		    ...
		    <div class="modal-dialog ">
		      <div class="modal-content">
		        ...
		        <div class="modal-body" style="text-align: center">
					...
					<table class="planeTbl" style="width:100%">
						<tr>
							<td style="text-align: right">Department Name <span class="req">*</span> </td>
							<td>
						...
						<tr>
							<td></td>
							<td><input type="button" value="Submit" class="btn" onclick="updateDept()"></td>
						</tr>
					</table>
					<br/>
...
<!-- 		        </div> -->
		        <div class="modal-body" style="text-align: center">
					<br/><br/>
		          	...
		          	<h3 style="color:green">New Department is created successfully</h3>
