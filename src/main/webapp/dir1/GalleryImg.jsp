...
<head>
	...
	function tableLoad(){
		...
		var galID=$("#gal").val();
		var subgalID=$("#subgal").val();
		$.ajax({
			type:"GET",
			url:"../GetGalleryPics",
			data:{"galid":galID, "subgalid":subgalID},
			success:function(data){
				...
				for (var key in data.hm) {
					($('#myTable tbody')).append('<tr >'
						+'<td>'+ ++sno+'</td>'
						+'<td style="padding:5px"> <img src="'+s3path+'gallery/'+galID+'/'+subgalID+'/'+data.hm[key]+'" style="max-width:100px"/> </td>'
						+'<td>'+data.hm[key]+'</td>'
	...
	function loadSubGallery(){
		...
		$.ajax({
			type:"GET",
			url:"../GetSubGallerysByGalID",
			data:{"galid":galID},
			success:function(data){
				($('#subgal')).append('<option value="" selected="selected">~~ Choose ~~</option>');
				for (var key in data.hm) {
					($('#subgal')).append('<option value="'+key+'">'+data.hm[key][2]+'</option>');
		        }
			},
			error:function(data){
...
// 		$('#imgDiv').show();
		...
		if($('#subgal').val() != "" && $('#gal').val() !=""){
			...
			alert("Select gallery and sub gallery both.");
		}
	  		...
	  		<br/>
	  			<table  class="planeTbl" style="width:100%">
	  				<tr>
	  					...
	  					<td style="width:10%;text-align: right">Sub Gallery <span class="req">*</span></td>
	  					<td style="width:40%">
	  						<select name="subgal" id="subgal" required class="form-control">
	  							<option value="" selected="selected">~~ Choose ~~</option>
	  						</select>
	  					</td>
	  				</tr>
	  				...
	  				<tr>
	  					...
	  					<td></td>
	  					<td>
	  						<input type="hidden" value="0" name="ID"/><input type="hidden" id="s3path" value="${s3path}">
...
<!-- 	  			<br/> -->
	  		...
	  		<div id="imgDiv" style="display:none">
	  			...
	  			<form action="" method="post" enctype="multipart/form-data">
		  			<table class="planeTbl bgcolor" style="width:60%;margin:auto;border:0px solid red">
		  				<tr>
		  					<td>Upload Images</td>
		  					<td><input type="file" name="pics" required multiple="multiple"> </td>
		  				</tr>
		  				<tr>
	  		...
	  		<form action="DeleteGalleryPics" method="post">
	  		<table id="myTable" class="Vscroll">
	  			<thead>
	  				<tr>
	  					<th style="color: #fff; background: #002e3b;border:1px solid #003135; ">Sr. No</th>
	  					<th style="color: #fff; background: #002e3b;border:1px solid #003135; ">Images</th>
