...
<head>
	...
	<script>
		function puValsInModel(dsid,did,examname,year)
		{ 
			alert(dsid+" "+did+" "+examname+" "+year);
			$("#dsid-modal").val(dsid);
			$("#did-modal").val(did);
			/* $("#dept-modal").val(deptname); */
			$("#name-modal").val(examname);
			$("#yr-modal").val(year);
	...
	<style>
		...
		.planeTbl tr:HOVER {
  		...
  		<div  style="border:1px solid #aaa;padding:10px;border-radius:5px;text-align: center">
	  		...
	  		<h3>Add Department Placement</h3>
	  		<br/>
	  		 <script>
	  		function chkPDF(){
  				var f=$('#f1').val();
	  		...
	  		<form action="deptPlacement" method="post"  enctype="multipart/form-data" onsubmit="return chkPDF()">
	  			<table  class="planeTbl" style="width:100%;">
	  				...
	  				<tr>
  						...
  						<td>Placement year:- <span class="req">*</span></td>
  		...
  		<table id="myTable" class="Vscroll">
  			<thead>
  				<tr>
  					...
  					<th style="color: #fff; background: #002e3b;border:1px solid #003135; ">Document Title</th>
  				    <th style="color: #fff; background: #002e3b;border:1px solid #003135; ">Placement year</th>
  				...
  				<c:forEach var="cat" varStatus="row" items="${det}">
  				 <tr>
  				     ...
  				     <td>${cat.examname}</td>
  				     <td>${cat.year}</td>
  				     <td><a href="${s3path}addPlacement/${cat.deptid}_${cat.document}" target="_blank">${cat.document}</a></td>
  				     <td><a href="DeptDeleteResult?id=${cat.said}">Delete</a></td>
  				     <td><input type="button" onclick="puValsInModel('${cat.said}','${cat.deptid}','${cat.examname}','${cat.year}')" value="  Edit  " class="green-btn" data-toggle="modal" data-target="#myModal"/></td>
  				 </tr>
  				</c:forEach>
  			</thead>
		        ...
		        <div class="modal-body" style="text-align: center">
		         <script>
	  			function chkPDF2(){
					...
					<form action="DeptResultUpdate" method="post"  enctype="multipart/form-data" onsubmit="return chkPDF2()">
		        	<table  class="planeTbl" style="width:100%">
		        		...
		        		<tr>
		        			...
		        			<td width="32%">
		  					...
		  					<td>Exam Name:-<span class="req">*</span></td>
