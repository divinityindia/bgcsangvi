...
<head>
	...
	<script>
		function puValsInModel(srid,did,title,details)
		{ 
			alert(srid+" "+did+" "+title+" "+details);
			$("#srid-modal").val(srid);
			$("#did-modal").val(did);
	...
	<style>
		...
		.planeTbl tr:HOVER {
  		...
  		<div  style="border:1px solid #aaa;padding:10px;border-radius:5px;text-align: center">
	  		...
	  		<h3>Add Department Research Publication</h3>
	  		<br/>
	  		<script>
	  			function chkPDF(){
	  				var f=$('#f1').val();
	  		...
	  		<form action="deptreserchpublication" method="post"  enctype="multipart/form-data" onsubmit="return chkPDF()">
	  			<table  class="planeTbl" style="width:100%">
	  				<tr>
	  					...
	  					<td><input type="text" name="rtitle" class="form-control" required="required"></td>
	  				</tr>
	  				<tr>
	  				    <td>Details:-</td>
	  				    <td><input type="text" name="rdetl" class="form-control" required="required"></td>
	  					<td></td>
	  					<td></td>
	  				</tr>
	  				<tr>
...
<!-- 					<input type="submit" value=" ADD " class="btn" /> -->
  		...
  		<br/>
  		<b>Already created departments:</b>
  			...
  			<thead>
  				...
  				<c:forEach var="cat" varStatus="row" items="${det}">
  				 <tr>
  				     ...
  				     <td>${cat.title }</td>
  				     <td>${cat.detalis }</td>
  				     <td><a href="https://s3.us-east-2.amazonaws.com/test-rmc-bucket/AddDepResPub/${cat.deptid }_${cat.document}">${cat.document}</a></td>
  				     <td><a href="DeptStaffResearchPrblDelete?id=${cat.srid}">Delete</a></td>
  				     <td><input type="button" onclick="puValsInModel('${cat.srid}','${cat.deptid}','${cat.title}','${cat.detalis}','${cat.document}')" value="  Edit  " class="green-btn" data-toggle="modal" data-target="#myModal"/></td>
  				 </tr>
  				</c:forEach>
  			</thead>
  			<tbody>
		    ...
		    <div class="modal-dialog modal-lg ">
		      <div class="modal-content">
		        <div class="modal-header">
		          ...
		          <h4 class="modal-title">Edit  Department Research Publication</h4>
		        </div>
		        ...
		        <script>
	  			function chkPDF2(){
					...
					<form action="DeptRserchPublicationUpdate" method="post"  enctype="multipart/form-data" onsubmit="return chkPDF2()">
		        	<table  class="planeTbl" style="width:100%">
		        		...
		        		<tr>
		        			...
		        			<td width="32%">
		  					...
		  					<td>Title:-<span class="req">*</span></td>
		  					<td><input type="text" class="form-control" required="required" style="resize:none" name="title" id="title-modal"> </td>
		  				</tr>
		  				<tr>
		  					<td>Details:- <span class="req">*</span></td>
		  					<td><input type="text" class="form-control"  id="dtl-modal" name="dtl" required="required"/></td>
		  				...
		  				<tr>
		  					<td></td>
		  					<td>
		  						<input type="hidden" name="srid" id="srid-modal"/>
