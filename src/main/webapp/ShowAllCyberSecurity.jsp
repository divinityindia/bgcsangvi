...
<!-- 							<li style="display: inline-block;" ><a href="CyberSecurity.jsp" >Home</a></li>-> -->
<%-- 							<li class="tg-active li" style="display: inline-block;">${type}</li> --%>
  ...
  <div class="container">
		...
		<style>
  		...
  		<table id="myTable" class="Vscroll">
  			<thead>
  				<tr>
  					<th>Sr. No</th><th>Semester</th><th>Document</th>
  			...
  			<c:forEach var="cat" items="${dtl}" varStatus="row">
  				<tr>
  				     <td>${row.count}</td>
  				     <td>${cat.semester}</td>
  				     <td><a href="${s3pat}CyberSecurity/${cat.csid}_${cat.document}" target="_blank">${cat.document}</a></td>
  				</tr>
  			</c:forEach>
